{{-- Menu Icon in Header --}}
<div class="relative -left-10 flex items-center gap-5">
    ...
    <a href="#" class="hover:scale-110 transition-transform">
        <svg width="22" height="21" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path
                ...
                fill="white" />
            <defs>
                <linearGradient id="paint0_linear_104_3472" x1="3.52207" y1="3.13754" x2="18.0808" y2="17.6963"
    ...
    @auth('customer')
        <li class="relative" x-data="{ open: false }">
            ...
            <button @click="open = !open"
                class="flex items-center gap-2 px-3 py-2 rounded-full hover:bg-gray-100 transition focus:outline-none">
            ...
            <div x-show="open" @click.away="open = false"
                class="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-lg ring-1 ring-black/5 z-50 overflow-hidden p-10">

                <!-- User Info -->
                <ul class="py-2 grid grid-cols-1 gap-5 items-center">
                    <li>
                        <a href="#" class="flex items-center gap-2 px-4 py-2 text-gray-700 hover:bg-gray-50 transition">
                    ...
                    <li>
                        <a href="#" class="flex items-center gap-2 px-4 py-2 text-gray-700 hover:bg-gray-50 transition">
                            {{-- <i class="bx bxl-gmail text-red-600 text-lg"></i> --}}
                            <img src="{{ Auth::guard('customer')->user()->profile
            ? asset('storage/' . Auth::guard('customer')->user()->profile)
            : asset('frontend/assets/imges/profile.png') }}" alt="Profile"
                    ...
                    <li class="border-t mt-2">
                        <form method="POST" action="{{ route('client.logout.submit') }}">
                            @csrf
                            <button type="submit"
                                class="flex items-center gap-2 w-full text-left px-4 py-4 text-red-600 hover:bg-red-50 transition font-medium">
...
<!-- HEADER -->
<header class="relative w-full flex flex-col items-center z-[9999]"> <!-- WRAPPER to control overlap -->
    <div class="relative w-full flex flex-col items-center"> <!-- TOP GOLD BAR -->
        <div class="relative top-[34px] w-[80%] h-[65px] flex items-center justify-between px-10 z-20"
            ...
            <!-- Logo Mobile left of background image top -->
            <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[50%]"> <img
                    src="{{ asset('frontend/assets/logo/logo_rice.png') }}" alt="Logo" class="h-14" /> </div>
            <!-- Left: Contact Info -->
            <div class="relative left-10 flex flex-wrap items-center gap-4 text-sm text-black font-medium">
                <span class="flex items-center gap-2"> <svg width="14" height="14" fill="currentColor"
                        viewBox="0 0 24 24">
                        <path
                    ...
                    </svg> info@lehsekmeasrice.com </span> <span class="flex items-center gap-2"> <svg width="14"
                        height="14" fill="currentColor" viewBox="0 0 24 24">
                        <path
                    ...
                    </svg> (+855) 93 666 153 / 12 666 153 </span>
            </div> <!-- Center Logo -->
            <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[50%]"> <img
                    src="{{ asset('frontend/assets/logo/logo_rice.png') }}" alt="Logo" class="h-14" /> </div>
            <!-- Right: Social Icons -->
            <div class="relative -left-10 flex items-center gap-5"> <a href="#"
                    class="hover:scale-110 transition-transform"> <svg width="21" height="21" viewBox="0 0 21 21"
                        fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M10.3559 0C4.64566 0 0 4.64566 0 10.3559C0 16.0656 4.64566 20.7117 10.3559 20.7117C16.0656 20.7117 20.7117 16.0656 20.7117 10.3559C20.7117 4.64566 16.0665 0 10.3559 0ZM12.9313 10.7204H11.2465V16.7256H8.74988C8.74988 16.7256 8.74988 13.4443 8.74988 10.7204H7.56312V8.59804H8.74988V7.22524C8.74988 6.24205 9.21708 4.70573 11.2694 4.70573L13.1194 4.71282V6.77306C13.1194 6.77306 11.9952 6.77306 11.7766 6.77306C11.5581 6.77306 11.2473 6.88235 11.2473 7.35122V8.59846H13.1494L12.9313 10.7204Z"
                            fill="#007BFF" />
                    </svg> </a> <a href="#" class="hover:scale-110 transition-transform"> <svg width="22" height="21"
                    ...
                    <a href="#" class="hover:scale-110 transition-transform"> <svg width="22" height="21"
                        ...
                        <path
                            ...
                            fill="white" />
                        <defs>
                            <linearGradient id="paint0_linear_104_3472" x1="3.52207" y1="3.13754" x2="18.0808"
                                y2="17.6963" gradientUnits="userSpaceOnUse">
                                <stop stop-color="#FAE100" />
                                <stop offset="0.15" stop-color="#FCB720" />
                                <stop offset="0.3" stop-color="#FF7950" />
                                <stop offset="0.5" stop-color="#FF1C74" />
                  ...
                  {{-- User dropdown --}} 
                  @auth('customer')
                        <li class="relative" x-data="{ open: false }"> <!-- Profile Button --> <button @click="open = !open"
                                class="flex items-center gap-2 px-3 py-2 rounded-full hover:bg-gray-100 transition focus:outline-none">
                                <i class="menu-icon tf-icons bx bx-user text-xl"></i> <span
                                    class="hidden md:inline font-medium text-gray-700">
                                    {{ Auth::guard('customer')->user()->username }} </span> <i
                                    class="bx bx-chevron-down text-lg text-gray-500"></i> </button>
                            <!-- Dropdown Menu -->
                            <div x-show="open" @click.away="open = false"
                                class="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-lg ring-1 ring-black/5 z-50 overflow-hidden p-10">
                                <!-- User Info -->
                                <ul class="py-2 grid grid-cols-1 gap-5 items-center">
                                    <li> <a href="#"
                                            ...
                                            {{-- <i class="bx bx-user text-gray-500 text-lg"></i> <span
                                                class="block font-semibold text-sm"> {{
                                                Auth::guard('customer')->user()->username }} </span> --}} </a> </li>
                                    <li> <a href="#"
                                            class="flex items-center gap-2 px-4 py-2 text-gray-700 hover:bg-gray-50 transition">
                                            {{-- <i class="bx bxl-gmail text-red-600 text-lg"></i> --}} <img
                                                src="{{ Auth::guard('customer')->user()->profile ? asset('storage/' . Auth::guard('customer')->user()->profile) : asset('frontend/assets/imges/profile.png') }}"
                                                alt="Profile" class="w-[40px] h-[40px] rounded-full object-cover">
                                            {{ Auth::guard('customer')->user()->email }} </span> </a> </li>
                                    <li class="border-t mt-2">
                                        <form method="POST" action="{{ route('client.logout.submit') }}"> @csrf <button
                                                type="submit"
                                                class="flex items-center gap-2 w-full text-left px-4 py-4 text-red-600 hover:bg-red-50 transition font-medium">
                                                <i class="bx bx-log-out text-lg"></i> Logout </button> </form>
                                    </li>
                                </ul>
                            </div>
                    </li> @endauth
            </div>
        </div> <!-- BOTTOM WHITE BAR -->
        <div class="relative w-[65%] h-[130px] top-[-30px] flex justify-center items-center overflow-hidden z-10"
            style=" background-image: url('{{ asset('frontend/assets/imges/header-white.png') }}'); background-size: 100% 100%; background-repeat: no-repeat; background-position: center; ">
            <ul class="relative top-[30px] flex flex-wrap justify-center gap-8 text-green-700 font-medium mt-8">
                {{-- button mobile menu left --}} <li> <button class="hidden"> <img
                            src="{{ asset('image/icon/button-mobail.png') }}" alt="" class="w-[30px] h-[30px]">
                    </button> </li> {{-- button mobile change to close --}} <li> <button class="hidden"> <img
                            src="{{ asset('image/icon/close.png') }}" alt="" class="w-[30px] h-[30px]">
                    </button> </li>
